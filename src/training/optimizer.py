...
def fetch_optimizer(args, params, hparams):
    ...
    if args.optimizer == "Adam":
        optimizer = Adam(
            params, lr=hparams.learning_rate, weight_decay=params.weight_decay
        )
        ...
        optimizer = AdamP(
            params, lr=hparams.learning_rate, betas=(0.9, 0.999), weight_decay=1e-2
        )
    else:
        NotImplementedError
...
def fetch_scheduler(args, optimizer):
    ...
    if args.scheduler == "CosineAnnealingLR":
        ...
        scheduler = GradualWarmupSchedulerV2(
            optimizer, multiplier=1, total_epoch=1, after_scheduler=scheduler_cosine
        )
    elif args.scheduler == "CosWarm":
        scheduler = CosineAnnealingWarmRestarts(
            optimizer, T_0=10, T_mult=1, eta_min=args.eta_min, last_epoch=-1
        )
    else:
        NotImplementedError
